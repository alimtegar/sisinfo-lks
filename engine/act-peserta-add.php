	...
	if(isset($_POST['submit-daftar'])){
		...
		&& isset($_POST['passwd-conf'])){
			...
			if($query_peserta && $query_nilai){
				header('location:../nilai.php?i=Tambah+peserta+sukses');
			}else{ header('location:../nilai.php?w=Terdapat+kesalah+dalam+perintah%2c+harap+coba+lagi'); }
		}else{ header('location:../nilai.php?w=Kolom+ada+yang+kosong%2c+harap+coba+lagi'); }
	}else{ header('location:../nilai.php?w=Tambah+peserta+gagal%2c+harap+coba+lagi'); }
 ?>