<?php 
	$query_peserta = mysqli_query($mysqli, "SELECT * FROM peserta");

	if(isset($_GET['cari-peserta']) || isset($_GET['cari-bidang'])){
		$cari_peserta = $_GET['cari-peserta'];

		$query_peserta = mysqli_query($mysqli, "SELECT * FROM peserta WHERE id LIKE '%$cari_peserta%' OR nama LIKE '%$cari_peserta%'");
	}
 ?>
<section id="nilai">
	<div class="section-container">
		...
		<div class="section-body">
			<?php if(mysqli_num_rows($query_peserta)){ ?>
			<?php if(isset($akses) && $akses == "admin"){ ?>
			...
			<table>
				...
				<tbody>
					<?php
						...
						while($result_peserta = mysqli_fetch_array($query_peserta)){
							$id = $result_peserta['id'];
							$nama = $result_peserta['nama'];
							$bidang = $result_peserta['bidang'];
							$asalsmk = $result_peserta['asalsmk'];

							$query_nilai = mysqli_query($mysqli, "SELECT * FROM nilai WHERE id='$id'");
			...
			<?php }else{ ?>
			<p class="info noresult">Peserta tidak dapat ditemukan/belum ada. <a href="javascript:void(0)" onclick="history.back()">Klik untuk kembali</a>.</p>
			<?php } ?>
		</div>
	</div>
</section>
		...
		<div class="section-body">
			<form method="POST" action="engine/act-peserta-add.php">
				<div class="form-first">
					...
					<input type="text" name="nama" placeholder="Nama Lengkap *" required onchange="passwdConf()"><br>
					<input type="password" name="passwd" placeholder="Kata Sandi *" required onchange="passwdConf()"><br>
					<input type="password" name="passwd-conf" placeholder="Ulangi Kata Sandi *" required><br>
